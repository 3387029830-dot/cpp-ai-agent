...
TEST_CASE("PermissionManager asks before write tools in ask-each-time mode") {
    bool prompted = false;
    PermissionManager permissions(PermissionMode::AskEachTime, [&](const PermissionRequest& request) {
        prompted = true;
        CHECK(request.preview == "diff preview");
        return true;
    });

    CHECK(permissions.approve({"edit_file", RiskLevel::Write, R"({"path":"x"})", "diff preview"}));
    CHECK(prompted);
}
