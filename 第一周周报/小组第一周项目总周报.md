...
| 魏宇鑫 | LLM 接入、AgentLoop 主循环、tool_calls 解析、上下文管理与 AGENTS.md 规范注入 | `src/llm`、`src/agent`、`src/core`、`AGENTS.md` |
| 康艺凡 | 工具系统、安全确认、路径越界防护、diff preview、JSONL 日志与历史回放 | `src/tools`、`src/security`、`src/storage`、测试报告 |
| 王博轩 | Console UI、Skill、MCP 最小客户端、Web 搜索、扩展能力说明 | `src/ui`、`src/skills`、`src/mcp`、`src/tools/WebSearchTool.*` |
